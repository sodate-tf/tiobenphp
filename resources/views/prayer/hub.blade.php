...
@section('title', $meta['title'] ?? 'Oração Católica Prática — IA Tio Ben')
@section('meta_description', $meta['description'] ?? '')
@section('canonical', $meta['canonical'] ?? url('/oracao-catolica-pratica'))
@section('robots', $meta['robots'] ?? 'index,follow,max-image-preview:large')
@section('og_title', $meta['og_title'] ?? 'Oração Católica Prática — IA Tio Ben')
@section('og_description', $meta['og_description'] ?? ($meta['description'] ?? ''))
@section('og_url', $meta['og_url'] ?? url('/oracao-catolica-pratica'))
...
@push('head')
  ...
  {!! json_encode([
    ...
    '@type' => 'CollectionPage',
    'name' => 'Oração Católica Prática',
    'description' => $meta['description'] ?? '',
    'url' => $meta['canonical'] ?? url('/oracao-catolica-pratica'),
...
<main class="bg-slate-50/60 pb-14">
  <header class="border-b bg-white">
    <div class="mx-auto max-w-6xl px-4 py-9">
      <nav class="text-xs text-slate-500" aria-label="Breadcrumb">
        ...
        <span class="mx-1">›</span>
        <span class="font-semibold text-slate-800">Oração Católica Prática</span>
      </nav>

      <div class="mt-5 max-w-4xl">
        <p class="text-xs font-black uppercase tracking-[0.18em] text-emerald-800">Hub especial</p>
        <h1 class="mt-2 text-3xl font-black tracking-tight text-slate-950 md:text-5xl">Oração Católica Prática</h1>
        <p class="mt-4 text-base leading-relaxed text-slate-600 md:text-lg">
          Conteúdos para transformar oração em rotina: terço, silêncio, perseverança, foco e passos concretos para viver a fé no dia a dia.
        </p>
      </div>

      <div class="mt-5 flex flex-wrap gap-2">
        @foreach(['Terço do dia', 'Rotina de oração', 'Silêncio interior', 'Vida sacramental', 'Perseverança', 'Família e oração'] as $tag)
          <span class="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-950">{{ $tag }}</span>
        @endforeach
      </div>
    </div>
  </header>
  ...
  <section class="mx-auto max-w-6xl px-4 py-8">
    @if($featured)
      <article class="overflow-hidden rounded-[2rem] border border-emerald-200 bg-white shadow-sm">
        <a href="{{ url('/blog/'.$featured->slug) }}" class="group grid gap-0 md:grid-cols-[minmax(0,1fr)_360px]">
          <div class="aspect-[16/9] bg-emerald-100 md:aspect-auto">
            @if($cover($featured))
          ...
          <div class="flex flex-col justify-center p-6 md:p-8">
            <p class="text-xs font-black uppercase tracking-[0.18em] text-emerald-800">Comece por aqui</p>
            ...
            @endif
            <span class="mt-5 text-sm font-black text-emerald-900">Ler artigo →</span>
          </div>
        </a>
      </article>
    @endif
